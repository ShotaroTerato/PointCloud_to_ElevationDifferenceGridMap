...
{
  ros::init(argc, argv, "PC_to_occmap");
  ROS_WARN("activate");
  ros::NodeHandle nh;
  ...
  if(pcl::io::loadPCDFile<pcl::PointXYZ>("/home/tera/pcdfile/gym_edit.pcd", *pcl_pc_in) == -1){
  ...
  z_max_ = voxel_pc->points[0].z;
  z_min_ = voxel_pc->points[0].z;
  for(int i=0; i<pc_size_; i++){
    ...
    if(z_max_ < voxel_pc->points[i].z) z_max_ = voxel_pc->points[i].z;
    if(z_min_ > voxel_pc->points[i].z) z_min_ = voxel_pc->points[i].z;
  }
  ...
  double passthrough_hight = z_min_ + lidar_hight;
      ...
      for(l=0; l<pc_size_; l++){
        if(cell[j][k].origin_x <= voxel_pc->points[l].x && voxel_pc->points[l].x <= cell[j][k].origin_x+resolution_){ 
          if(cell[j][k].origin_y <= voxel_pc->points[l].y && voxel_pc->points[l].y <= cell[j][k].origin_y+resolution_){
            if(voxel_pc->points[l].z <= passthrough_hight + 0.1 && voxel_pc->points[l].z >= passthrough_hight - 0.0){
              cell[j][k].diff = occ_value;
            }else if(!(cell[j][k].diff == occ_value)){
              cell[j][k].diff = 0;
            }
  ...
  cv::imwrite("/home/tera/occmap.jpg", pc_img);
